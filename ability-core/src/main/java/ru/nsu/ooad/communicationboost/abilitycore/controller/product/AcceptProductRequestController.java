...
public class AcceptProductRequestController implements IHandler {

    private final ProductRequestService productRequestService;

    @Value(value = "${commands.accept.product.request.name}")
    ...
    public ResponseEntity<MessageResponse> executeCommand(DtoMarker interModularDto) {
        return ResponseEntity.ok(productRequestService.acceptProductRequest(interModularDto));
    }

    @Override
    public MetaInfo getMetaInformation() {
        return new MetaInfo(UtilConsts.CommandConsts.ACCEPT_PRODUCT_REQUEST_URL, commandName);
    }
