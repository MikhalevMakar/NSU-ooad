...
import ru.nsu.ooad.communicationboost.abilitycore.dto.annotations.ProductRequestDeletion;
...
public class CancelProductRequestController implements IHandler {

    private final ProductRequestService productRequestService;

    @Value(value = "${commands.cancel.product.request.name}")
    ...
    @Override
    @ProductRequestDeletion
    public ResponseEntity<MessageResponse> executeCommand(DtoMarker interModularDto) {
        return ResponseEntity.ok(productRequestService.cancelProductRequest(interModularDto));
    }

    @Override
    public MetaInfo getMetaInformation() {
        return new MetaInfo(UtilConsts.CommandConsts.CANCEL_PRODUCT_REQUEST_URL, commandName);
    }
