...
import ru.nsu.ooad.communicationboost.abilitycore.dto.annotations.Username;
...
public class UnblockUserController implements IHandler {

    private final UserService userService;

    @Value(value = "${commands.unblock.user.name}")
    private String commandName;

    @Override
    @Username
    public ResponseEntity<MessageResponse> executeCommand(DtoMarker interModularDto) {
        return ResponseEntity.ok(userService.unblockUser(interModularDto));
    }

    @Override
    public MetaInfo getMetaInformation() {
        return new MetaInfo(UtilConsts.CommandConsts.UNBLOCK_USER_URL, commandName);
    }
